...
def count_trainable_params(model: AutoModel) -> dict[str, int]:
    def module_trainable(module) -> int:
        return sum(p.numel() for p in module.parameters() if p.requires_grad)

    lora_trainable = module_trainable(model.language_model)
    projector_trainable = module_trainable(model.mlp1)
    vision_trainable = module_trainable(model.vision_model)
...
def print_trainable_summary(model: AutoModel, train_projector: bool) -> None:
    stats = count_trainable_params(model)
    pct = 100 * stats["total_trainable"] / stats["total_params"]
    print("Trainable parameter summary:")
    print(f"  LoRA (language_model): {stats['lora_trainable']:,}")
    print(f"  Projector (mlp1):      {stats['projector_trainable']:,}  (enabled={train_projector})")
    print(f"  Vision (should be 0):  {stats['vision_trainable']:,}")
    print(f"  Total trainable:       {stats['total_trainable']:,} / {stats['total_params']:,} ({pct:.4f}%)")
    if train_projector and stats["projector_trainable"] == 0:
        raise RuntimeError("train_projector=True but mlp1 has no trainable parameters.")
    if stats["vision_trainable"] != 0:
        raise RuntimeError("vision_model should remain frozen during LoRA SFT.")
...
def apply_lora(
    ...
    print_trainable_summary(model, train_projector=train_projector)
    return model
...
def build_trainer_callbacks(cfg: TrainConfig) -> list:
